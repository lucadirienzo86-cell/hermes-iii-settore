...
export interface TipiAgevolazioneOption {
  id: string;
  nome: string;
  ordine: number | null;
  attivo: boolean | null;
...
export const useTipiAgevolazioneOptionsAdmin = () => {
  const queryClient = useQueryClient();

  const { data: options = [], isLoading } = useQuery({
    queryKey: ["tipi-agevolazione-options-admin"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("tipi_agevolazione_options")
        .select("*")
        .order("ordine", { ascending: true });

      if (error) throw error;
      return data as TipiAgevolazioneOption[];
    },
  });
  ...
  const addOption = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tipi-agevolazione-options-admin"] });
