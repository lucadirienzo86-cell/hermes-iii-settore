...
const ServiceLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!roles.includes("service" as any) && !roles.includes("facility" as any)) {
        await supabase.auth.signOut();
        throw new Error("Accesso non autorizzato. Questo portale è riservato ai fornitori di servizi.");
      }

      toast({
        title: "Accesso effettuato",
        description: "Benvenuto nel portale Service!",
      });
    ...
    <div className="min-h-screen bg-[#f4f7f9] flex flex-col items-center justify-center px-4 py-12">
      ...
      <Card className="w-full max-w-md border-[#e1e8ed] shadow-md">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl text-[#2c3e50]">Accesso Service / Facility</CardTitle>
      ...
      <p className="text-center text-xs text-[#7f8c9a] mt-6">
        Portale riservato ai fornitori di servizi e facility
      </p>
    </div>
  );
};
