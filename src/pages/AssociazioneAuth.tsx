...
import { Heart, ArrowLeft, LogIn, UserPlus, Loader2 } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';

const AssociazioneAuth = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Verify user is an associazione
      const { data: roleData } = await supabase
        .from('user_roles')
        .select('role')
        .eq('user_id', data.user.id);

      const hasAssociazioneRole = roleData?.some(r => r.role === ('associazione' as any));

      if (!hasAssociazioneRole) {
        await supabase.auth.signOut();
        throw new Error('Questo account non è registrato come Associazione');
      }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const redirectUrl = `${window.location.origin}/associazione/dashboard`;
      ...
      if (data.user) {
        // Create the associazione record
        const { error: assocError } = await supabase
          .from('associazioni_terzo_settore')
          .insert({
            profile_id: data.user.id,
            denominazione: registerData.denominazione || 'Associazione',
            email: registerData.email,
            tipologia: 'ETS',
        ...
        if (assocError) {
          console.error('Errore creazione associazione:', assocError);
        }
      }
    ...
    <div className="min-h-screen bg-[#f4f7f9] flex flex-col items-center justify-center px-4 py-12">
      ...
      <Card className="w-full max-w-md border-[#e1e8ed] shadow-md">
        <CardHeader className="text-center pb-2">
          <CardTitle className="text-2xl text-[#2c3e50]">Accesso Associazione</CardTitle>
          <CardDescription className="text-[#5a6c7d]">
            Accedi o registra la tua Associazione
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="register">
              <form onSubmit={handleRegister} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="reg-denominazione" className="text-[#2c3e50]">Denominazione Associazione *</Label>
                  <Input
      ...
      <p className="text-center text-xs text-[#7f8c9a] mt-6">
        Portale riservato alle Associazioni del Terzo Settore
      </p>
    </div>
  );
};

export default AssociazioneAuth;
